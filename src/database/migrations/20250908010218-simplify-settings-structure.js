...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      ...
      // ✅ IMPORTANT: Drop settings_audit FIRST (it references pricing_rules)
      ...
      // Then drop pricing_rules
      const [tables] = await queryInterface.sequelize.query("SHOW TABLES LIKE 'pricing_rules'", {
        transaction,
      });
    ...
    try {
      ...
      if (!tableDescription.customer_tier) {
        await queryInterface.addColumn(
          'business_settings',
          'customer_tier',
          { type: Sequelize.ENUM('regular', 'business', 'premium'), allowNull: true },
          { transaction }
        );
      }
